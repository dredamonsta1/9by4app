// src/components/Rooms/Rooms.jsx — Live rooms lobby
...
function Rooms() {
  ...
  useEffect(() => {
    fetchRooms();
    // Poll for live participant counts every 15s
  ...
  const handleRoomCreated = (room) => {
    setShowCreate(false);
    // Navigate into the room immediately (we already have a token from creation)
    navigate(`/rooms/${room.room_id}`, { state: { token: room.token, role: "host" } });
  };
    ...
    <div className={styles.container}>
      <div className={styles.content}>
        <div className={styles.header}>
          ...
          {user && (
            <button className={styles.startBtn} onClick={() => setShowCreate(true)}>
              + Start Room
            </button>
          )}
        </div>
